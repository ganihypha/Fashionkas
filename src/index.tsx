...
app.get('/sw.js', async (c) => {
  const sw = `const CACHE_NAME='fashionkas-v2.5';self.addEventListener('install',()=>{self.skipWaiting()});self.addEventListener('activate',(e)=>{e.waitUntil(caches.keys().then(k=>Promise.all(k.filter(x=>x!==CACHE_NAME).map(x=>caches.delete(x)))));self.clients.claim()});self.addEventListener('fetch',(e)=>{if(e.request.method!=='GET'||new URL(e.request.url).pathname.startsWith('/api/'))return;e.respondWith(fetch(e.request).then(r=>{if(r.ok){const c=r.clone();caches.open(CACHE_NAME).then(ca=>ca.put(e.request,c))}return r}).catch(()=>caches.match(e.request).then(c=>c||new Response('Offline',{status:503}))))});`;
...
// Health check
app.get('/api/health', (c) => c.json({ status: 'ok', app: 'FashionKas', version: '2.5', webhook: '/api/webhook/incoming', features: ['catalog', 'kasir', 'orders', 'wa-automation', 'r2-upload', 'fonnte-bot', 'subscriptions'] }))
